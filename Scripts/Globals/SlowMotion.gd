...
func slow_motion(
	...
	var _must_keep_slowest_speed := false
	if current_state != States.IDLE:
		match custom_overlap_mode:
			...
			OverlapModes.REPLACE_IF_LONGER_USE_NEW_SPEED:
				var _remaining_duration: float = target_duration - current_elapsed_time
	...
	# Update ongoing effect
		...
		var _expected_start_of_recover_os_time: float = start_os_time + recover_time
