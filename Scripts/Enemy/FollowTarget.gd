...
func go_to_location(targetPositions : Vector3):
	if not isTargetSet or lastTargetPosition != targetPositions:
		set_target_position(targetPositions)
		lastTargetPosition = targetPositions
		isTargetSet = true
